...
void run_diferential_evolution_for_fjssp (char *filename_of_FJSSP_instance, int NP, float F, float CR, double PLS, double PLSm);
...
void machines_vector_backward_conversion (double *individual_machines_half, int *permutation_vector, int **machines_alternatives, int *assigned_machines_vector, int d, int number_of_jobs, int *amount_of_machines_per_operations, int *number_operations_per_job, double *individual_lower_bound, double *individual_upper_bound);
...
main (
    ...
    double PLSm; //probability of local search machines vector
    if (argc == 7) {
        ...
        PLSm = (convert_string_argv_to_int (argv, 6) / 100.0);
    }
    else {
        printf ("use: ./differentialEvolution.out filename_of_FJSSP_instance NP F(*100) CR(*100) PLS(*100) PLSm(*100)");
        exit (1);
    }
    ...
    run_diferential_evolution_for_fjssp (filename_of_FJSSP_instance, NP, F, CR, PLS, PLSm);
}

void 
run_diferential_evolution_for_fjssp (
    char *filename_of_FJSSP_instance, int NP, float F, float CR, double PLS, double PLSm
    ) {
    ...
    readInstanceFJJ (
        filename_of_FJSSP_instance, &job_data, &number_of_machines, &number_of_jobs, 
        &number_operations_per_job, &job_id_x_operation_id, &number_of_operations
        );
    ...
    generate_machine_alternatives_vector_per_operations(
        D, job_data, number_of_machines, amount_of_machines_per_operations, machines_alternatives
        );
    ...
    initialize_individuals_randomly (
        population, lower_bound, upper_bound, individuals_fitness, NP, D, job_data, number_operations_per_job, number_of_machines, 
        number_of_jobs, amount_of_machines_per_operations, machines_alternatives
        );

    total_iter = 0;
    do {
        DE_mutate_recombine_evaluate_and_select (
            population, individuals_fitness, NP, D, F, CR, job_data, number_operations_per_job, number_of_machines, 
            number_of_jobs, amount_of_machines_per_operations, machines_alternatives, upper_bound
            );
        ...
        total_of_evaluation_in_local_search += run_machine_change_local_search (
            population, NP, D, PLS, individuals_fitness, job_data, number_operations_per_job, 
            number_of_machines, number_of_jobs, amount_of_machines_per_operations, machines_alternatives, 
            lower_bound, upper_bound
            );
        total_of_evaluation_in_local_search += run_local_search (
            population, NP, D, PLSm, individuals_fitness, job_data, number_operations_per_job, 
            number_of_machines, number_of_jobs, amount_of_machines_per_operations, machines_alternatives, 
            lower_bound, upper_bound
            );
    ...
    print_best_individual (
        best_individual, D, number_operations_per_job, number_of_jobs, 
        amount_of_machines_per_operations, machines_alternatives, upper_bound
        );
...
void 
generate_machine_alternatives_vector_per_operations(
    int D, int **job_data, int number_of_machines, int *amount_of_machines_per_operations, int **machines_alternatives
    ) {
...
void 
initialize_individuals_randomly (
    double **population, double *lower_bound, double *upper_bound, double *individuals_fitness, int NP, int D, int **job_data, 
    int *number_operations_per_job, int number_of_machines, int number_of_jobs, int *amount_of_machines_per_operations, int **machines_alternatives
    ) {
    ...
    for (i=0; i<NP; i++) {
        ...
        individuals_fitness[i] = DE_evaluate (
            population[i], D, job_data, number_operations_per_job, number_of_machines, number_of_jobs, 
            amount_of_machines_per_operations, machines_alternatives, upper_bound
            );
    }
}
...
void 
DE_mutate_recombine_evaluate_and_select (
    double **population, double *individuals_fitness, int NP, int D, float F, float CR, int **job_data, int *number_operations_per_job, 
    int number_of_machines, int number_of_jobs, int *amount_of_machines_per_operations, int **machines_alternatives, double *upper_bound
    ) {
    ...
    #pragma omp parallel
    {
        ...
        for (i=0; i<NP; i++) {
            ...
            trials_fitness[i] = DE_evaluate (
                trial_population[i], D, job_data, number_operations_per_job, number_of_machines, number_of_jobs, 
                amount_of_machines_per_operations, machines_alternatives, upper_bound
                );
...
void 
DE_mutate_and_recombine (
    double **population, int individual_index, double *trial_vector, int NP, int D, float F, float CR
    ) {
...
void 
DE_select (
    double *individual, double *fitness_of_individual, double *trial_vector, double *fitness_of_trial_vector, int D
    ) {
...
double 
DE_evaluate (
    double *individual, int D, int **job_data, int *number_operations_per_job, int number_of_machines, int number_of_jobs, 
    int *amount_of_machines_per_operations, int **machines_alternatives, double *upper_bound
    ) {
    ...
    int *assigned_machines_vector = generate_machines_vector(
        individual_machines_half, d, amount_of_machines_per_operations, machines_alternatives, 
        upper_bound, solution->job, number_operations_per_job, number_of_jobs
        );

    fitness = evaluate_with_machine_vector(
        *solution, current_fitness , number_of_machines, number_of_jobs, d, number_operations_per_job, 
        job_data, evaluation_up_to_date, assigned_machines_vector
        );
...
int* 
decode_solution (
    double *individual_operations_half, int d, int *number_operations_per_job, int number_of_jobs
    ) {
...
int* 
init_permutation_vector_for_individual_operations (
    double *individual, int d
    ) {
...
void 
change_permutation_vector_to_permutation_with_repetitions (
    int *permutation_vector, int d, int *number_operations_per_job, int number_of_jobs
    ) {
...
int* 
generate_machines_vector(
    double *individual_machines_half, int d, int *amount_of_machines_per_operations, int **machines_alternatives, 
    double *upper_bound, int *permutation_vector, int *number_operations_per_job, int number_of_jobs
    ) {

    int *assigned_machines_vector = init_int_array (d);
    int i, machine_to_use, job_index;
    int index_of_operation_in_machines = 0, index_to_start = 0;
    int *operations_processed_index_per_job = init_int_array (number_of_jobs);

    for (i=0; i < number_of_jobs; i++) {
        operations_processed_index_per_job[i] = index_to_start;
        index_to_start += number_operations_per_job[i];
    }

    for (i=0; i<d; i++) { 
        job_index = permutation_vector[i] - 1; // job_number - 1
        index_of_operation_in_machines = operations_processed_index_per_job[job_index];
        operations_processed_index_per_job[job_index] += 1;
        ...
        if (amount_of_machines_per_operations[index_of_operation_in_machines] == 1) {
            ...
            machine_to_use = select_machine_to_use(
                individual_machines_half[i], amount_of_machines_per_operations[index_of_operation_in_machines], 
                upper_bound[index_of_operation_in_machines]
                );
            assigned_machines_vector[i] = machines_alternatives[index_of_operation_in_machines][machine_to_use];
        }
    }

    free (operations_processed_index_per_job);
    operations_processed_index_per_job = NULL;
    return assigned_machines_vector;
}

int 
select_machine_to_use(
    double xj, int amount_of_machines, double upper_bound
    ) {
    ...
    return (int) machine_to_use;
}

int 
run_local_search (
    double **population, int NP, int D, double PLS, double *individuals_fitness, int **job_data, 
    int *number_operations_per_job, int number_of_machines, int number_of_jobs, int *amount_of_machines_per_operations, 
    int **machines_alternatives, double *individual_lower_bound, double *individual_upper_bound
    ) {

    int i, number_of_evaluations_done = 0;
    int lower_bound, upper_bound, index1, index2;
    lower_bound = D / 2; // D is double dimension. We only want to operations vector changes.
    upper_bound = D - 1;
    ...
    #pragma omp parallel
    {
        ...
        for (i=0; i<NP; i++){
            if (drand48() < PLS){
                ...
                LS_evaluate_and_select (
                    population[i], D, &individuals_fitness[i], index1, index2, job_data, 
                    number_operations_per_job, number_of_machines, number_of_jobs, 
                    amount_of_machines_per_operations, machines_alternatives, individual_upper_bound
                    );
        ...
        for (i=0; i<NP; i++){
            if (drand48() < PLS){
                ...
                while (j<D && finish_LS_for_this_individual == false){
                    ...
                    finish_LS_for_this_individual = LS_evaluate_and_select (
                        population[i], D, &individuals_fitness[i], index1, index2, job_data, 
                        number_operations_per_job, number_of_machines, number_of_jobs, 
                        amount_of_machines_per_operations, machines_alternatives, individual_upper_bound
                        );
...
bool 
LS_evaluate_and_select (
    double *individual, int D, double *fitness_of_individual, int index1, int index2, int **job_data, 
    int *number_operations_per_job, int number_of_machines, int number_of_jobs, 
    int *amount_of_machines_per_operations, int **machines_alternatives, double *individual_upper_bound
    ) {
    ...
    trial_fitness = DE_evaluate (
        trial_individual, D, job_data, number_operations_per_job, number_of_machines, 
        number_of_jobs, amount_of_machines_per_operations, machines_alternatives, individual_upper_bound
        );
...
int 
run_machine_change_local_search (
    double **population, int NP, int D, double PLS, double *individuals_fitness, int **job_data, 
    int *number_operations_per_job, int number_of_machines, int number_of_jobs, 
    int *amount_of_machines_per_operations, int **machines_alternatives,
    double *individual_lower_bound, double *individual_upper_bound
    ) {
...
void 
machine_change_local_search (
    double *individual, int d, int *number_operations_per_job, int number_of_jobs, 
    int *amount_of_machines_per_operations, int **machines_alternatives, 
    double *individual_lower_bound, double *individual_upper_bound
    ) {
    ...
    assigned_machines_vector = generate_machines_vector(
        individual_machines_half, d, amount_of_machines_per_operations, machines_alternatives, 
        individual_upper_bound, permutation_vector, number_operations_per_job, number_of_jobs
        );

    assigned_machines_vector = change_to_better_machine_in_one_job(
        assigned_machines_vector, d, amount_of_machines_per_operations, machines_alternatives, 
        permutation_vector, number_operations_per_job
        );
    
    machines_vector_backward_conversion (
        individual_machines_half, permutation_vector, machines_alternatives, assigned_machines_vector, 
        d, number_of_jobs, amount_of_machines_per_operations, number_operations_per_job, 
        individual_lower_bound, individual_upper_bound
        );
...
int* 
change_to_better_machine_in_one_job (
    int *assigned_machines_vector, int d, int *amount_of_machines_per_operations, 
    int **machines_alternatives, int *permutation_vector, int *number_operations_per_job
    ) {
...
int 
calculate_index_of_operation_in_machines (
    int *permutation_vector, int index_operation_to_alter, int *number_operations_per_job
    ) {
...
std::vector<int> 
get_new_possible_machines_for_this_operation (
    int index_operation_to_alter, int *possible_machines_this_operation, 
    int amount_of_machines_this_operation, int *assigned_machines_vector
    ) {
...
void 
machines_vector_backward_conversion (
    double *individual_machines_half, int *permutation_vector, int **machines_alternatives, 
    int *assigned_machines_vector, int d, int number_of_jobs, int *amount_of_machines_per_operations, 
    int *number_operations_per_job, double *individual_lower_bound, double *individual_upper_bound
    ) {

    int i, j, job_index, selected_machine;
    double rj, lj;
    int *possible_machines_this_operation;
    int *operations_processed_index_per_job = init_int_array (number_of_jobs);
    int index_of_operation_in_machines = 0, index_to_start = 0;
    ...
    for (i=0; i<d; i++) { 
        ...
        if (lj == 1) {
            ...
            possible_machines_this_operation = machines_alternatives[index_of_operation_in_machines];
            rj = 0;
            selected_machine = assigned_machines_vector[i];
            for (j=0; j<lj; j++) { 
                if (possible_machines_this_operation[j] == selected_machine) {
                    rj = j + 1 ; // +1 because j is the index of array
                }
            }
            individual_machines_half[i] = ((2 * individual_upper_bound[i]) / lj) * rj - individual_upper_bound[i];
        }
    }
...
double 
best_fitness_of_population (
    double **population, double *individuals_fitness, int NP, double *best_individual, int D
    ) {
...
void 
copy_population (
    double **source, double **destination, int NP, int D
    ) {
...
void 
copy_individual (
    double *source, double *destination, int D
    ) {
...
void 
print_population (
    double **population, int NP, int D
    ) {
...
void 
bubbleSort (
    double *array, int *id_array, int n
    ) { 
...
double** 
init_matrix_with_value (
    int number_rows, int number_columns, double value
    ) {
...
double** 
init_matrix (
    int number_rows, int number_columns
    ) {
...
void 
set_all_matrix_values_to (
    double **matrix, int number_rows, int number_columns, double value
    ) {
...
void 
print_best_individual (
    double *best_individual, int D, int *number_operations_per_job, int number_of_jobs, 
    int *amount_of_machines_per_operations, int **machines_alternatives, double *upper_bound
    ) {
    ...
    int *best_individual_decode = decode_solution (
        best_individual_operations_half, d, number_operations_per_job, number_of_jobs
        );
    int *best_assigned_machines_vector = generate_machines_vector(
        best_individual_machines_half, d, amount_of_machines_per_operations, machines_alternatives, 
        upper_bound, best_individual_decode, number_operations_per_job, number_of_jobs
        );
